...
const char* ReadCommand(const char* current, int* pos_in_buffer, int* type, int* value)
{
    ...
    #ifdef DEBUG
        printf("DEBUG: ReadCommand data: '%s'\n", data);
    #endif
...
Node_t* LoadTreeFromFile(Tree_t* tree, const char** buffer, int* pos_in_buffer, LoadProgress* progress, const char* buffer_start)
{
    ...
    const char* current = SkipSpaces (*buffer, pos_in_buffer);
        ...
        LOADING_BASE_DUMP(tree, *buffer, *pos_in_buffer, "Start making node", progress);
    #endif
    ...
    if (*current == '(')
    {
        ...
        if (!node)
        {
            ...
            LOADING_BASE_DUMP(tree, *buffer, *pos_in_buffer, "Node created", progress);
        #endif

        if (progress) AddNodeToLoadProgress(progress, node);
        ...
        if (*current == '(')
        {
                ...
                LOADING_BASE_DUMP(tree, *buffer, *pos_in_buffer, "before making LEFT subtree", progress);
            #endif
            ...
            node->left = LoadTreeFromFile (tree, &current, pos_in_buffer, progress, buffer_start);
                ...
                LOADING_BASE_DUMP(tree, *buffer, *pos_in_buffer, "LEFT subtree completed", progress);
            #endif
        }
        ...
        if (*current == '(')
        {
                ...
                LOADING_BASE_DUMP (tree, *buffer, *pos_in_buffer, "before making RIGHT subtree", progress);
            #endif
            ...
            node->right = LoadTreeFromFile (tree, &current, pos_in_buffer, progress, buffer_start);
                ...
                LOADING_BASE_DUMP (tree, *buffer, *pos_in_buffer, "RIGHT subtree completed", progress);
            #endif
        }
        else if (strncmp(current, "nil", 3) == 0)
        {
        ...
        *buffer = current;
            ...
            LOADING_BASE_DUMP(tree, *buffer, *pos_in_buffer, "Node FULL BUILT", progress);
        #endif
...
TreeErr_t LoadDatabase(Tree_t* tree, const char* filename)
{
    ...
    tree->root = LoadTreeFromFile(tree, &buffer_ptr, &position, &progress, buffer);
...
{
    TreeInsertRoot(tree, OPERATORTYPE, MUL);
    TreeInsertLeft(tree, tree->root, VARIABLETYPE, ARGX);
    TreeInsertRight(tree, tree->root, OPERATORTYPE, ADD);
    TreeInsertLeft(tree, tree->root->right, VARIABLETYPE, ARGY);
    TreeInsertRight(tree, tree->root->right, NUMBERTYPE, 6);
}
